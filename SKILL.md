...
Connects an OpenClaw agent to an [Outline](https://www.getoutline.com/) wiki instance via the REST API.
Configured for `wiki.thakaa.cloud` by default — override via env file.

## Setup

1. Run the installer:
   ```bash
   bash scripts/install.sh /path/to/credentials/outline.env
   ```

2. Or manually set credentials at `/data/openclaw/credentials/outline.env` (or any path):
   ```env
...
3. Test:
   ```bash
   bash /path/to/scripts/outline.sh collections
   ```

## Tool

All commands via: `bash /path/to/scripts/outline.sh <command>`

Set `OUTLINE_SCRIPT` in your agent config to point to the installed path.
